...
class PatchDataset(Dataset):
    def __init__(self, root_dir, output_dir, transform=None):
        self.root_dir = root_dir
        ...
        print(f"   👉 Found total {total_files} images.")
        print("   🧹 Filtering processed files...")

        # 2. [핵심] 이미 .pt 파일이 있는 것은 리스트에 넣지 않음 (Skip)
        for img_path in tqdm(all_files, desc="Checking existence"):
            # 저장될 경로 미리 계산
            rel_path = os.path.relpath(img_path, self.root_dir)
            save_rel_path = os.path.splitext(rel_path)[0] + ".pt"
            save_full_path = os.path.join(self.output_dir, save_rel_path)
            
            # 파일이 없을 때만 추가! (이미 있으면 패스)
            if not os.path.exists(save_full_path):
                self.image_paths.append(img_path)
        
        remaining = len(self.image_paths)
        skipped = total_files - remaining
        print(f"   ⏩ Skipped {skipped} files. Only processing {remaining} files.")

    def __len__(self):
        return len(self.image_paths)

    def __getitem__(self, idx):
        # 여기는 건드릴 필요 없음 (남은 파일만 들어오니까)
        img_path = self.image_paths[idx]
        try:
            image = Image.open(img_path).convert('RGB')
            if self.transform:
...
def main():
    ...
    dataset = PatchDataset(args.input_dir, transform=transform, output_dir=args.output_dir)
    ...
    with torch.no_grad():
        for images, paths in tqdm(dataloader):
            ...
            for i, path in enumerate(paths):
                ...
                # if there's the file, skip 
                # if not os.path.exists(os.path.join(args.output_dir, save_rel_path)):
...
CUDA_VISIBLE_DEVICES=0 python extract_features.py --input_dir /mnt/j/brain/TS --output_dir /mnt/d/workspace/dataset/brain/TS_UNI --model uni --batch_size 128;
CUDA_VISIBLE_DEVICES=1 python extract_features.py --input_dir /mnt/j/brain/BS --output_dir /mnt/d/workspace/dataset/brain/BS_UNI --model uni --batch_size 128;

CUDA_VISIBLE_DEVICES=1 python extract_features.py --input_dir /mnt/j/brain/TS --output_dir /mnt/d/workspace/dataset/brain/TS_Virchow2 --model virchow2 --batch_size 128;
CUDA_VISIBLE_DEVICES=1 python extract_features.py --input_dir /mnt/j/brain/BS --output_dir /mnt/d/workspace/dataset/brain/BS_Virchow2 --model virchow2 --batch_size 128;

CUDA_VISIBLE_DEVICES=1 python extract_features.py --input_dir /mnt/j/brain/TS --output_dir /mnt/d/workspace/dataset/brain/TS_CONCH --model conch --batch_size 128;
CUDA_VISIBLE_DEVICES=1 python extract_features.py --input_dir /mnt/j/brain/BS --output_dir /mnt/d/workspace/dataset/brain/BS_CONCH --model conch --batch_size 128;
